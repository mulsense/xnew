...
| `xnew.state.capture(root)` | server サブツリーを `SyncNode[]`（state tree）として捕捉。 |
| `xnew.state.apply(root, tree)` | client サブツリーへ差分適用（create/update/remove）。 |
...
- **capture**（`xnew.state.capture`）: server サブツリーを pre-order DFS し `SyncNode[]` を全量返す。
- **reconcile**（`xnew.state.apply`）: root ごとの `Map<id, Unit>`(WeakMap) を保持し、
  - create: 未知 id → `getRegisteredComponent(name)` → `xnew(parent, Component)`（mode は親 client を継承）→ state 適用 → map 登録（pre-order なので親が先）。
