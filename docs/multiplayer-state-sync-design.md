...
| API | 役割 |
| --- | --- |
| `xnew.boot(mode, ...args)` | エンジンの mode（`'server' \| 'client' \| null`）を適用して `xnew(...args)` でルートを生成し、終了後（例外時も）前の値へ復元。**mode を選ぶ唯一の公開手段**。生成した Unit を返す。 |
...
- `Unit.config = { mode: null }`（static、内部のみ）。**公開設定手段は `xnew.boot` だけ**で、
  `xnew.config` は公開しない（`xnew.boot` が `Unit.config.mode` を一時設定／復元する）。
- unit の mode 解決: `parent ? (parent._.mode ?? Unit.config.mode ?? null) : null`。
  - engine root は null。サブツリーのルート（親が null-mode）が `Unit.config.mode` を採用し、
    ネストした子は親の非 null mode を継承。
- **`Unit.reset()` は `Unit.config.mode` をクリアしない**（初回 `xnew()` が自動 reset を走らせるため、
  クリアすると設定済み mode が生成前に消える）。`xnew.boot` の `finally` が前の値へ戻す。
...
const server = xnew.boot('server', Main);
const client = xnew.boot('client', Main);
