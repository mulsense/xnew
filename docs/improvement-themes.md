...
- **影響:** 中 / **コスト:** 大 / **互換性:** △（破壊的の可能性） / **ステータス:** 未着手

### 現状・問題

- mode 選択の唯一の公開口が `xnew.boot('server', …)`（[src/core/xnew.ts](../src/core/xnew.ts)）で発見しにくい。
- `xnew.server()` / `xnew.client()` は名前がゲッターのようだが、実体は「extend 風の条件実行ブロック」。
- `Unit.config.mode` がグローバル可変状態。
  [src/core/unit.ts](../src/core/unit.ts) のコメントが自ら「reset でクリアしない」要注意点を説明している＝設計上の脆さ。

### 提案

- `xnew.server(fn)` → `xnew.onServer(fn)` 等、動詞性のある名前に。
- `config.mode` のグローバル依存を縮小し、構築時に明示的に渡す設計へ寄せる。
