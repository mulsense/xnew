...
- **影響:** 大（TS 利用時） / **コスト:** 中 / **互換性:** ◎（実行時不変） / **ステータス:** 未着手
...
- `xnew.extend` / `xnew.server` / `xnew.client` の戻り値型も同様に合成するか。
- props 型 `P` を `xnew(Component, props)` 側の引数と連動させる（誤った props を弾く）か。
- `Unit & A` のプロパティ名衝突（既存の `on`/`element` 等）の扱い。
...
|---|---|---|---|---|
| 2 型付き defines/Component | 大 | 中 | ◎ | 未着手 |
