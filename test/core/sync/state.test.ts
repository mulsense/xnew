...
describe('xnew.state.initialize', () => {
    ...
    it('registers syncState on the current unit and returns the same reference', () => {
        ...
        const unit = xnew((u: Unit) => { state = xnew.state.initialize({ position: 0 }); });
    ...
    it('merges on repeated calls, keeping the same reference', () => {
        ...
        xnew((u: Unit) => { s1 = xnew.state.initialize({ a: 1 }); s2 = xnew.state.initialize({ b: 2 }); });
        expect(s1).toBe(s2);
