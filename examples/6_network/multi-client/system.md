...
**差し替え点はただ 1 つ**: `xnew.sync.boot(mode, transport, ...)` に渡す transport（と、server を同一プロセスで boot するか別プロセスにするか）。
...
- **状態の下り** … `xnew.sync.boot(mode, transport, World)` に transport を渡すと、boot が自動で配線する。
...
| `xnew.sync.boot(mode, transport, ...)` | 起動コード | （io / io() の用意） | transport を渡すと socket を自動バインドし、状態の下り(capture→broadcast / apply)も自動配線 |
...
xnew.sync.boot('server', transport, World);          // 擬似サーバー
xnew.sync.boot('client', transport, stage, World);   // 擬似クライアント（左）
xnew.sync.boot('client', transport, stage, World);   // 擬似クライアント（右）
```
...
socket.once('connect', () => {
  ...
  xnew.sync.boot('client', transport, stage, World);    // World は無改変
});

// server（server.js / Node）
const transport = xnew.sync.socketio(io);          // ← transport だけ差し替え
xnew.sync.boot('server', transport, World);             // World は無改変
```
...
// boot(mode, transport, World) に transport を渡すと、capture→broadcast / 受信→apply を boot が自動配線する
...
| `xnew.sync.boot(mode, transport, ...)` | transport を渡すと socket を自動バインド・clientId を自動発番し、状態の下り(capture→broadcast / apply)も自動配線 |
