...
|---|---|---|---|---|---|
| **browser-only** | `index-browser-only.html` | `xnew.sync.loopback()` | ブラウザ内（擬似） | ファイルを開くだけ | 単一 World |
...
**socket.io（実ネット）**: transport を `xnew.sync.socketio(...)` に差し替える。ルーム対応なので
...
// server（server.js / Node）: ルームごとに room スコープ transport + boot
const transport = xnew.sync.socketio(io, { room: roomId });
xnew.sync.boot(transport.server, World);        // auto-mirror は io.to(roomId) にだけ broadcast
```
...
サーバーは **ルームごとに独立した World ツリー**を持つ。肝はルーム単位の transport:

```js
// server.js: ルーム作成時
const transport = xnew.sync.socketio(io, { room: id });   // query.room===id の接続だけ扱う
const root = xnew.sync.boot(transport.server, World);      // auto-mirror は io.to(id) にだけ broadcast
// 人数は transport.server.on('connect'/'disconnect') で数え、空室は猶予後に root.finalize() で掃除
```
