...
> server→client 同期（`xnew.boot` / `xnew.server` / `xnew.client` / `xnew.sync.state|register|mirror`）
...
xnew.boot('server', World);                     // 擬似サーバー
xnew.boot('client', stage, World);              // 擬似クライアント（左）
xnew.boot('client', stage, World);              // 擬似クライアント（右）
```
