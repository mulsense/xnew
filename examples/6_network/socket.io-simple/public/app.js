...
//
// welcome で受け取った gameType の共有モジュールを動的 import し、xnew.sync.boot(socket, World) で
...
function GameScene(unit, { state, roomId }) {
    ...
    gameSocket.on('welcome', async ({ id, field, roomId: rid, roomName, gameType }) => {
        ...
        // boot は socket から mode を判定する。下りは gameSocket.on('sync') で手動 apply するため、
        // boot の auto-mirror は使わない。in-process loopback を渡して client モードだけ得る（空回り）。
        const mod = await import(`/games/${gameType}.js`);
        clientWorld = xnew.sync.boot(xnew.sync.loopback().connect(), unit, mod.World, { selfId: id, field });
