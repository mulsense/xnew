...
- **プラグインの契約**（`games/*.js` が export）：`id` / `name` / `create()` と、共有コンポーネント `World` / `Player`。`create()` は server ツリーを `xnew.sync.boot('server', World)` でブートし、`onJoin`（Player を spawn）/ `onInput`（入力 ref を更新）/ `onLeave`（finalize）/ `capture()`（`xnew.sync.capture`）/ `dispose()` を実装する。**ゲームを増やすのは `games/` にファイルを足すだけ**。
