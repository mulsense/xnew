...
- **状態同期は xnew.sync**: ルームごとに server ツリー（`xnew.boot('server', World)`）と client ツリー（ブラウザの `xnew.boot('client', World)`）を持つ。`Player` は **server/client 共有の1コンポーネント**で、`xnew.server` ブロックが移動を計算し `xnew.client` ブロックが `<div>` アバターを描く。サーバーは `xnew.sync.capture(World)` した state tree を配り、ブラウザは `xnew.sync.apply(clientWorld, tree)` で差分反映する。
