...
**`xnew.sync.boot(socket, ...args)`** で行う(mode を選ぶ唯一の公開手段。socket のメンバから mode を判定する)。その mode で `xnew(...args)` を
生成する。in-process のデモは `xnew.sync.loopback()` で server↔client を繋ぐ:

```js
const transport = xnew.sync.loopback();
const server = xnew.sync.boot(transport.server, Main);    // この Main とその子孫 = server
const client = xnew.sync.boot(transport.connect(), Main); // この Main とその子孫 = client
```
...
| 要素 | 役割 |
|---|---|
| `xnew.sync.boot(socket, ...args)` | socket から mode を判定し xnew(...args) を生成(server/client を選ぶ) |
