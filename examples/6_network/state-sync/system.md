...
**`xnew.sync.boot(mode, transport, ...args)`** で行う(mode を選ぶ唯一の公開手段。transport は標準ネットワーク用、無ければ `null`)。その mode で `xnew(...args)` を
生成し、終わると mode が前の値へ自動で戻る:

```js
const server = xnew.sync.boot('server', null, Main);   // この Main とその子孫 = server
const client = xnew.sync.boot('client', null, Main);   // この Main とその子孫 = client
```
...
| 要素 | 役割 |
|---|---|
| `xnew.sync.boot(mode, transport, ...args)` | その mode で xnew(...args) を生成(server/client を選ぶ。transport 無しは null) |
