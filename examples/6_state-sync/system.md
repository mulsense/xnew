...
## 5. xnew.state.capture — server ツリーを「撮影」
...
## 6. xnew.state.apply(reconcile)— client ツリーを「差分で」更新
...
| `xnew.state.capture` | server ツリー → SyncNode 配列(撮影) |
| `xnew.state.apply` | SyncNode 配列 → client ツリーへ差分反映 |
