...
**`xnew.boot(mode, fn)`** で行う(mode を選ぶ唯一の公開手段)。`fn` の実行中だけその mode が
適用され、終わると前の値へ自動で戻る:

```js
const server = xnew.boot('server', () => xnew(Main));   // この Main とその子孫 = server
const client = xnew.boot('client', () => xnew(Main));   // この Main とその子孫 = client
```
...
| 要素 | 役割 |
|---|---|
| `xnew.boot(mode, fn)` | fn 実行中だけ mode を適用してツリーを生成(server/client を選ぶ) |
