...
function Texture(unit, { texture, offset = { x: 0, y: 0} }) {
...
function GameScene(unit, { level }) {
  ...
  xnew.context('gamestate', gameState);
  ...
  xnew(Background);
  xnew(Floor);
  xnew(Texture, { texture: xpixi.sync(xthree.canvas), offset: { x: 0, y: -60 } });
      ...
      xnew.timeout(() => {
        xnew.listener(window).on('keydown pointerdown', () => {
          ...
          if (level + 1 < levels.length) {
              xnew.append(Main, GameScene, { level: level + 1 });
          } else {
...
function Floor(unit) {
  ...
  for (let y = 0; y < GRID; y++) {
    for (let x = 0; x < GRID; x++) {
      ...
      const color = (x + y) % 2 === 0 ? 0xCCCCCC : 0x444444;
...
function InfoPanel(unit) {
  const gameState = xnew.context('gamestate');
