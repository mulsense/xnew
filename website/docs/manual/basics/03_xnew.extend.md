...
`xnew.extend` は別のコンポーネント関数を現在の unit のコンテキストで実行し、その戻り値を現在の unit にマージします。新しい要素は生成せず、振る舞いだけをミックスインするための関数です。

ドラッグ・ロギング・キーボード操作などの機能をコンポーネントとして用意しておけば、他のコンポーネントに 1 行で組み込めます。
