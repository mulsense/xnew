...
`xnew.extend` は別のコンポーネント関数を現在の unit のコンテキストで実行し、その戻り値を unit にマージします。新しい要素は生成せず、振る舞いだけをミックスインします。

ドラッグ・ロギング・キーボード操作などの機能をコンポーネントとして書いておけば、他のコンポーネントに 1 行で組み込めます。
...
**パラメータ:**
- `Component`: 拡張に用いるコンポーネント関数
- `props`: コンポーネント関数に渡す任意のプロパティ

**動作:**
- 現在の unit のコンテキストでコンポーネント関数を即時実行します。
- 戻り値のプロパティ/メソッドを現在の unit にマージします。
- イベントハンドラは累積されます。後の拡張で上書きされません。
- 同じプロパティ名を返した場合、後の拡張が優先されます。
...
ドラッグ処理を独立したコンポーネントに切り出し、再利用できる形にする例です。
