...
`xnew.find` は、指定したコンポーネント関数で生成された、現在アクティブなすべての unit を返します。配列を手動で管理せずに、すべての敵への一斉通知、カウンターの一括同期、複数オブジェクトとの衝突判定などを行えます。
...
- **フィルタリング**: 条件に合うインスタンスを抽出する

:::tip
`xnew.find` が返すのはアクティブな unit のみです。finalize 済みの unit は含まれません。
:::
