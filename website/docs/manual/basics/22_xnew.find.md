...
`xnew.find` は、指定したコンポーネント関数で生成された、現在アクティブなすべての unit を返します。配列を手動で管理せずに、すべての敵にブロードキャストしたり、カウンターを一括同期したり、複数オブジェクトとの衝突判定を行ったりできます。
...
`xnew.find` は次のような用途に適しています。

- **複数インスタンスの一括操作**: 同種のコンポーネントすべてを更新・制御
- **状態の同期**: 複数コンポーネントを同期した状態に保つ
- **ブロードキャスト**: すべてのインスタンスにメッセージを送信
- **クリーンアップ**: 全インスタンスを検索してまとめて削除
- **統計と監視**: アクティブなインスタンス数の集計や分析
- **フィルタリングと選択**: 条件に合うインスタンスの抽出

:::tip
`xnew.find` はアクティブな unit のみを返します。finalize された unit は含まれません。
:::
