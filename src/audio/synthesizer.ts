...
export function synthesizer(props: SynthProps) {
...
class Synthesizer {
    ...
    press(frequency: number | string, duration?: number | string, wait?: number) {
        ...
        const freq: number = typeof frequency === 'string' ? keymap[frequency] : frequency;
        const dura: number = typeof duration === 'string' ? (notemap[duration] * 60 / (props.bpm ?? 120)) : (typeof duration === 'number' ? (duration / 1000) : 0);
