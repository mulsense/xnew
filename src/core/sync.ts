...
// - RoomInfo / ServeRoomsOptions / serveRooms : ロビー + 動的ルームのサーバー配線
//----------------------------------------------------------------------------------------------------
...
/**
 * socket.io の io にロビー + 動的ルームを配線する。接続は query.room の有無で 2 系統:
 *   - ロビー（room 無し）: lobby:rooms（一覧配信）/ room:create → room:created | room:error
 *   - ゲーム（room 付き）: room スコープの transport で boot 済みの component に参加
 * ルームごとに boot するので状態はルーム間で混ざらない。空室は猶予後に root.finalize() で掃除。
 */
export function serveRooms(io: any, options: ServeRoomsOptions): void {
    ...
    function createRoom(rawName: string): { room?: RoomState; error?: string } {
        ...
        // room スコープの transport を作り、その server socket で component を boot する。
